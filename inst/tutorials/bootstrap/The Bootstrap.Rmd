...
The reading for this module discussed a variety of non-parametric statistical techniques for comparing means. This tutorial focuses on one, the bootstrap, and walks you through how to  use the technique to estimate the [standard error](https://en.wikipedia.org/wiki/Standard_error) (SE) not just for the mean but for *any* sample statistic. 
...
```{r include =F}
d <- read_csv("data/flight_delays.csv")
d <- d[1:360,] %>% 
  mutate(delay = as.numeric(`Arrival delay in minutes`),
         delay = delay + rnorm(n = 360, sd = .75),
