...
description: >
  Learn to use logistic regression to do binary classification, and practice converting results into reportable quanitities.
---
...
- The modeling pipeline using the `caret` package.
- Interpreting logistic regression coefficients.
- The confusion matrix and the ROC curve for comparing comparing classification models.
- Comparing models using estimated out-of-sample error metrics.
...
```{r}
data.frame(Variable = c("College", "Income",  "Overage_charge", "Leftover", "House", "Handset_price", "Over_15mins_calls_per_month", "Average_call_duration", "Reported_satisfaction", "Reported_usage_level", "Considering_change_of_plan", "Leave","ID"),
...
```{r x12_5, exercise = T, message =F, warning=F}
...
# Plot
ggroc(roc_object, alpha = 0.5, colour = "red", legacy.axes = T,
      levels = c("STAY","LEAVE")) +
...
roc(m$leave, predict(glm_model, type = "response"),
     levels = c("STAY","LEAVE"))  %>% 
  ggroc(alpha = 0.5, colour = "red", legacy.axes = T) +
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color="grey", linetype="dashed") +
  theme_minimal() +
...
Equipped with the above information we will practice the entire modeling pipeline in preparation for the case study.

- Do necessary cleaning.
- Create a set of predictors that are entirely numeric. Why do we do this? So we can impute missing observations with medians.
- Remove variables with near zero variance. This is an optional step. 
- Impute with medians.  This time we will impute outside of the `train()` function.
- Assess and remove correlated variables. This is also an optional step.
- Fit a logistic regression model of churn.
...
Remember to include the `fullRank = T` argument!

```{r x21_5, exercise = T, warning=FALSE}
# Set up dummy frame, remembering to remove id
dummy_frame <- dummyVars(leave~ ., fullRank = T, data = select(m, -id)) %>% 
  predict(newdata = m)
...
There are two near zero variance predictors identified by `nzv()`. 

```{r x24, exercise = T}
# Check for near zero variance predictors
nzv(dummy_frame, names = T) 
...
dummy_frame %>% 
  data.frame %>% 
  select(overage_charge, reported_usage_level.avg) %>% 
  head(20)
...
Once all the predictors are numeric, we can use impute missing observations. In this case we are imputing outside of the `train()` function in order to have complete cases for the next (optional) step in the modeling pipeline: finding and removing highly correlated predictors.  
...
```{r x27, exercise = T, warnings = F, messages = F}
# Fit multiple models.  This code is slow to run!
...
```{r x28, exercise = T, warning=F, message= F}
...
- An increase of one unit in centered and scaled `house` (i.e., one standard deviation)  is associated with a change of -.49 in the log odds of churn. (OR: $e^{-.49} = .61$, indicating about a 40% *decrease* in the odds of churn compared to the baseline of 1.)

- An increase of one unit in centered and scaled `overage_charge` (i.e., one standard deviation)  is associated with a change of .42 in the log odds of churn. (OR: $e^{.42} = 1.52$, indicating about a 52% *increase* in the odds of churn compared to the baseline of 1.)
